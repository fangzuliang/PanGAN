...
def main(argv):
    ...
    dataset=DataSet(FLAGES.pan_size, FLAGES.ms_size, FLAGES.img_path, FLAGES.data_path, FLAGES.batch_size,
                    FLAGES.num_spectrum, FLAGES.stride, FLAGES.norm)
    ...
    with tf.Session() as sess:
        ...
        if FLAGES.is_pretrained:
            saver.restore(sess, FLAGES.model_save_dir + '/PanNet-100')
        for training_itr in range(FLAGES.iters):
            ...
            for i in range(2):
                ...
                _, error_pan_model = sess.run([model.train_spatial_discrim, model.spatial_loss],
                                              feed_dict={model.pan_img: pan_batch, model.ms_img: ms_batch})
                _, error_ms_model = sess.run([model.train_spectrum_discrim, model.spatial_loss],
                                             feed_dict={model.pan_img: pan_batch, model.ms_img: ms_batch })
